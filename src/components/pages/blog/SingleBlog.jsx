...
const SingleBlog = () => {
    ...
    const handleLike = ()=> {
        if(isLiked){
            setHeart(heartImg);
            setIsLiked(false);
            setBlogToShow(prev=>({...prev, likes: prev.likes-1}))
        } else {
    ...
    const addUserComment = (e)=> {
        e.preventDefault();
        const newComment = new Comment(blogToShow.comments.length+1, userComment);
        setBlogToShow(prev=>({...prev, comments: [...prev.comments, newComment]}));
        setUserComment("");
    }

    const commentsBlock = blogToShow.comments.map(item=>{
        return <div key={item.id} className={classes.singleBlock__commentsBlock}>
    ...
    return (
        <div className={classes.singleBlock}>
            ...
            <img className={classes.singleBlock__img} src={blogToShow.img}/>
